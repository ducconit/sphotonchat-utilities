...
import (
	...
	translate "github.com/ducconit/go-translate"
...
func init() {
	...
	ToVi.Flags().String("replace", "", "Path to file of list string replace")
...
func toViF(command *cobra.Command, args []string) error {
	...
	converter.ids = ids
	converter.srcEn = src
	converter.bar = progressbar.Default(int64(len(ids)))
	converter.t = translate.NewGoogleTranslate()
...
func (c *convertToViFromEn) replaceMap(input interface{}) (interface{}, error) {
	if rows, ok := input.(map[string]interface{}); ok {
		result := make(map[string]interface{})
		for id, value := range rows {
			if valueStr, ok := value.(string); ok {
				text, err := c.t.Translate(valueStr)
				if err != nil {
					return nil, err
				}
				for search, replace := range c.replace {
